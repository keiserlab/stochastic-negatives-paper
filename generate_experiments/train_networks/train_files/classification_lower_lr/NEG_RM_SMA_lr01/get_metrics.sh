...
args_path=$HOME"/labgits/lab-notebook-caceres/Projects/nnets/20180815_Paper_Retrains/classification_lower_lr/NEG_RM_SMA_lr_01";
...
run_experiment () {
  ...
  validate="${val_script} ${output_dir} &> ${output_dir}/validation.log & pid=$!"  
...
for fold in {0..4}; do
  for pnr in ${sneg_pnrs[@]}; do
    run_experiment ${fold} ${NNET_BASE}/experiments/classification/classification_negs_rm.py ${pnr} CLASSIFIER_NEG_RM_SMA_RATIOS_LR01
  done
done
