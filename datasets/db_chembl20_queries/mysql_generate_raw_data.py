...
def activity_list(in_f, min_affinity=MIN_AFFINITY, max_affinity=MAX_AFFINITY,
                  ...
                  strong=False, all_binding_data=False, drugmat=False, for_sea=False):
    ...
    for line in in_f:
        ...
        if affinity is None:
            continue
        if (affinity < min_affinity or affinity > max_affinity) and for_sea:
            continue
        chembl_tid = splits[0]
        chembl_mid = splits[1]
        doc_id = splits[6]
...
if __name__ == "__main__":
    ...
    parser.add_argument('-m', '--max_affinity', type=float, 
                        ...
                        help="""Only include affinities up to X nM. Useful if SEA flag set""")
    ...
    matrix only.""")
        parser.add_argument('-s', '--for_sea', action='store_true', help="""Set to true to set max and min affinities for SEA""")
